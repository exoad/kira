...
enum class Modifiers(val tokenType: Token.Type, val wrappingContext: Array<WrappingContext> = emptyArray())
{
    ...
    REQUIRE(Token.Type.K_MODIFIER_REQUIRE, arrayOf(WrappingContext.CLASS_MEMBER, WrappingContext.VARIABLE, WrappingContext.FUNCTION)),
    WEAK(
        Token.Type.K_MODIFIER_WEAK,
        arrayOf(WrappingContext.CLASS_MEMBER, WrappingContext.VARIABLE, WrappingContext.FUNCTION, WrappingContext.FUNCTION_PARAMETER)
    )
    ;
