...
class KiraVisualViewer(private val context: SourceContext) : JFrame("Kira Lexer") {
    init {
        ...
        contentPane = JTabbedPane().apply {
            ...
            addTab("Token Location View", JScrollPane(renderTokenLocationView()).apply {
                isOpaque = true
                preferredSize = size
            })
        }
    ...
    private fun renderTokenLocationView(): JEditorPane {
        val editorPane = JEditorPane().apply {
            isEditable = false
            contentType = "text/html"
            font = Font(Font.MONOSPACED, Font.PLAIN, 14)
            ...
            node("html") {
                node(
                    ...
                    context.tokens.forEach { token ->
                        ...
                        {
                            append(
                                ("[${token.canonicalLocation} : ${token.canonicalLocation.column + token.content.length}]")
                                    .replace("&", "&amp;")
                                    .replace("<", "&lt;")
                                    .replace(">", "&gt;")
                                    .replace("\t", "&nbsp;&nbsp;&nbsp;&nbsp;")
